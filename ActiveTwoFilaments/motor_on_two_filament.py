...
#%% VARIABLES
folder_name = "Braun_2017_2911_1" # name of the simulation 
working_path = os.path.dirname(__file__)
folder = working_path  + "/"+ folder_name

N_save = 500 # number of save
timestep = 1e-2 #timestep
stop_time = 18*60 # max simulating time 
...
D_Ma = LS.Coefficient("D_Ma",0.1) # motors bound to (a)
D_Mb = LS.Coefficient("D_Mb",0.1) # motors bound to (b)
D_Mab = LS.Coefficient("D_Mab",0.05) # motors bound to (ab)
...
act = LS.Coefficient("act",1) # Activity coefficient withmotors
...
problem.add_equation("V_A = 1/(gamma.v+Coeff_fri)*( F_fA_ent +F_fA_act)")
...
while solver.proceed:
    ...
    # V_A['g'] = 0 
    # V_B['g'] = 0 
    Coeff_act['g'] = act.v*(0.5+0.5*np.tanh(0.05*(solver.sim_time-5*60)))*(0.5+0.5*np.tanh(-0.05*(solver.sim_time-13*60)))           
