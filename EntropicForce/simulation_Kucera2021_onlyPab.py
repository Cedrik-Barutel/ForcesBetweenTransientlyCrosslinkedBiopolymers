...
#%% VARIABLES
folder_name = "Kucera2021_2110_single_X1" # name of the simulation 
...
Lx = 1.5 #length of the simulation box 1 jumps
Nx = 2**9
...
D_Pab = LS.Coefficient("D_Pab",0.0088*1) # passive bound to (ab) 
...
Koff_1_Pa = LS.Coefficient("Koff_1_Pa",0.0) # Rates of reaction 1 around Ma_eq
...
problem.add_equation("dt(Pab)"
                      ...
                      "="
                      "-r*D_Pab.v*ddx(Pab)" # stabilization
                      "-D_Pab.v*dx( (f_B*Pab/(h+f_D)  )*dx(f_A))" # 
                      "-D_Pab.v*dx( (f_A*Pab/(h+f_D)  )*dx(f_B))" # 
...
name_file_velocity = "2210_VP_1_jump_s1"
file_ext = ".csv"
